...
// upload file
func uploadFile1(w ResponseWriter, r *Request, fs FileSystem, name string) {

    reader, err := r.MultipartReader()
    if err != nil {
		log.Printf("MultipartReader failed %s\n", err)
		return
    }
    ...
    for {
		...
		fh, err := os.Create(path.Join(dir, name))
		defer fh.Close()
		if err != nil {
			log.Printf("upload file filename=%s error, create file failed %s \n", name, err)
			return
		}
		log.Printf("POST [%s], FormName=[%s], FileName=[%s],\n", fh.Name(), part.FormName(), part.FileName())
		io.Copy(fh, part)
	}
		/*defer file.Close()
